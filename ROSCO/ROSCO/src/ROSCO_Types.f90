...
TYPE, PUBLIC :: ControlParameters
    ...
    REAL(DbKi)                    :: flp_bb_threshold            ! Threshold for flap mode = 4
    Real(DbKi)                    :: flp_bb_startDelay           ! Time into sim where DAC bang-bang controller activates (Flp_Mode = 4)
    Real(DbKi)                    :: flp_bb_depTime              ! Minimum amount of time a DAC device will deploy for when triggered (Flp_Mode = 4)
    INTEGER(IntKi)                :: flp_bb_useIDAC              ! A flag used to determine whether to use IDAC or general DAC in bang-bang flap control mode (Flp_Mode =4)
